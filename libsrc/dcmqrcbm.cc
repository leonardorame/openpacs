...
OFCondition DcmQueryRetrieveMoveContext::performMoveSubOp(DIC_UI sopClass, DIC_UI sopInstance, char *fname)
{
    ...
    // para que se aplique el codec correcto, hay que 
    // primero leer el dataset, y luego llamar a chooseRepresentation
    DCMQRDB_INFO("File: " << fname);
    DcmFileFormat dcmff;
...
OFCondition DcmQueryRetrieveMoveContext::buildSubAssociation(T_DIMSE_C_MoveRQ *request)
{
    ...
    DCMQRDB_INFO("AET Destino: " <<  request->MoveDestination);
    ...
    if (cond.good()) {
        ...
        DCMQRDB_INFO("---> Destination: " << dstHostNamePlusPort);
...
#else
    DCMQRDB_INFO("-- ENABLE_COMPRESSION_EXTENSION --");
