...
int DcmQueryRetrieveConfig::readConfigLines(FILE *cnffp)
{
   ...
   while (!feof(cnffp)) {
      ...
      if (!strcmp("ApplicationTitle", mnemonic)) {
      ...
      else if (!strcmp("ConnectionString", mnemonic)) {
        c = parsevalues(&valueptr);
        connectionString_ = c;
        free(c);
      }
        /*custom griensu*/
      else if (!strcmp("ThumbSize", mnemonic)) {
        c = parsevalues(&valueptr);
   ...
   else if (!strcmp("thumbPath", mnemonic)) {
        c = parsevalues(&valueptr);
        ...
        thumbPath_ = "no se ha  asignado un path para los thubnails";
        }
...
int DcmQueryRetrieveConfig::leerHostTable() 
{
   /* 
       Descargo: Si alguien lee este código quiero dejar 
       en claro que NO LO HICE YO (bah, una parte si).

                                     -- Leonardo M. Ramé
   */

  DCMQRDB_INFO("------------->>>> " << getConnectionString());


    int queryPos = 0;
    std::string lConn;
    std::string lSql = "select name,ip,port,aetitle from externalsystem";
    lConn = getConnectionString();
    PGconn *dbconn = PQconnectdb(lConn.c_str());
    if(PQstatus(dbconn) == CONNECTION_OK) 
    {
      PGresult * internalResult = PQexec(dbconn, lSql.c_str());
      queryPos = 0;
      ...
      {
        DCMQRDB_INFO("---  SIN DATOS ---");
      }
      else
      {
        DCMQRDB_INFO("---  Antes del bucle ---");

        // reservamos memoria para el host entry
        CNF_HETable.noOfHostEntries = PQntuples(internalResult);
        DcmQueryRetrieveConfigHostEntry *helpentry;
        if ((helpentry = (DcmQueryRetrieveConfigHostEntry *)malloc(CNF_HETable.noOfHostEntries * sizeof(DcmQueryRetrieveConfigHostEntry))) == NULL)
           panic("Memory allocation 1 (%d)", CNF_HETable.noOfHostEntries);
        CNF_HETable.HostEntries = helpentry;

        while (PQntuples(internalResult) > queryPos ){
          DCMQRDB_INFO("---  Dentro del bucle ---");
          CNF_HETable.HostEntries[queryPos].SymbolicName = PQgetvalue(internalResult, queryPos, PQfnumber(internalResult, "name") );

          // reservamos memoria para el peer list
          DcmQueryRetrieveConfigPeer *helppeer = NULL;
          if ((helppeer = (DcmQueryRetrieveConfigPeer *)malloc(sizeof(DcmQueryRetrieveConfigPeer))) == NULL)
            panic("Memory allocation 5 (%d)", 1);

          CNF_HETable.HostEntries[queryPos].noOfPeers = 1;
          CNF_HETable.HostEntries[queryPos].Peers = helppeer;
          CNF_HETable.HostEntries[queryPos].Peers[0].ApplicationTitle = PQgetvalue(internalResult, queryPos, PQfnumber(internalResult, "aetitle") );
          CNF_HETable.HostEntries[queryPos].Peers[0].HostName = PQgetvalue(internalResult, queryPos, PQfnumber(internalResult, "ip") );
          CNF_HETable.HostEntries[queryPos].Peers[0].PortNumber = atoi(PQgetvalue(internalResult, queryPos, PQfnumber(internalResult, "port")));
          DCMQRDB_INFO("AE"<< CNF_HETable.HostEntries[queryPos].Peers[0].ApplicationTitle);
          queryPos++;        
        }
      }
    }
...
int DcmQueryRetrieveConfig::peerForAETitle(const char *AETitle, const char **HostName, int *PortNumber) const
{
    ...
    if(PQstatus(dbconn) == CONNECTION_OK) 
    {
      ...
      if(PQntuples(internalResult) == 0)
      {
        DCMQRDB_INFO("--- No se han encontrado datos ---");
      }
      else
      {
        *HostName = PQgetvalue(internalResult, queryPos, PQfnumber(internalResult, "ip") );
        *PortNumber = atoi(PQgetvalue(internalResult, queryPos, PQfnumber(internalResult, "port") ));
         DCMQRDB_INFO("---> IP: " << PQgetvalue(internalResult, queryPos, PQfnumber(internalResult, "ip") ));
        return(1);
      }
    }
    else
...
const char *DcmQueryRetrieveConfig::getConnectionString() const
{
   return(connectionString_.c_str());
}
//griensu
const char *DcmQueryRetrieveConfig::getThumbSize() const
{
   return(thumbSize_.c_str());
}
const char *DcmQueryRetrieveConfig::getThumbPath() const
{
   return(thumbPath_.c_str());
}
...
DCMQRDB_INFO("---> checkForSameVendor: ");
...
int DcmQueryRetrieveConfig::HostNamesForVendor(const char *Vendor, const char ***HostNameArray) const
{
   int  i, j,
    found = 0;
   DCMQRDB_INFO("---> HostNamesForVendor: ");
   for(i = 0; i < CNF_VendorTable.noOfHostEntries; i++) {
...
{
    DCMQRDB_INFO("---> writableStorageArea: ");
