 ...
 *  Module:  openpacs
 *
 *  Author:  Leonardo M. Ramé (martinrame@yahoo.com)
 *
 *  Purpose: DICOM SCP Server connected to a PostgreSql Database
 *  This software is based on DCMTK's dcmqrscp (http://dcmtk.org)
 *
 */
...
#define OFFIS_CONSOLE_APPLICATION "openpacs"
#endif
...
#define APPLICATIONTITLE "OpenPACS"

const char *opt_configFileName = DEFAULT_CONFIGURATION_DIR "openpacs.cfg";
...
main(int argc, char *argv[])
{
  ...
  OFString temp_str;
#ifdef HAVE_FORK
  OFConsoleApplication app(OFFIS_CONSOLE_APPLICATION, "OpenPACS", rcsid);
#else
  OFConsoleApplication app(OFFIS_CONSOLE_APPLICATION, "OpenPACS\nThis version of OpenPACS supports only single process mode.", rcsid);
#endif
